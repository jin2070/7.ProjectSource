...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
  ...
  //  NSLog(@"btnMove.y:%f",self.buttonMovePoint.y);
    //通知启动
    // 1.添加字典, 将数据包到字典中
    NSNumber *numPointX=[NSNumber numberWithFloat:offsetX];//用通知传送Point
    NSNumber *numPointY=[NSNumber numberWithFloat:offsetY];//用通知传送Point
    //   NSLog(@"numPoint:%@",numPoint);
    NSDictionary *dict =[[NSDictionary alloc] initWithObjectsAndKeys:numPointX,@"offsetX",numPointY,@"offsetY", nil];
    // 2.创建通知
...
//@property (nonatomic, weak) ScrollviewFunction *itemScrollView;
...
- (void)initialize {
    ...
    ScrollviewFunction *itemScrollView = [[ScrollviewFunction alloc] init];
...
- (void)layoutSubviews {
    ...
    self.itemScrollView.backgroundColor=[UIColor grayColor];
...
- (void)resetSetupTrackerFrameWithSelectedButton:(UIButton *)selectedButton {
    ...
    //      NSLog(@"resetSetupTrackerFrameWithSelected %d",i);
    //NSLog(@"offSetX: %f,Maxoffset:%f",offSetX,maxOffsetX);
    CGFloat selectedButtonWidth = selectedButton.frame.size.width;
    //  NSLog(@"selectedButton.frame.size.width:%f",selectedButton.frame.size.width);
    switch (self.trackerStyle) {
        case SPPageMenuTrackerStyleLine:
        {
            NSLog(@"A:SPPageMenuTrackerStyleLine");
        ...
        {
            NSLog(@"B:SPPageMenuTrackerStyleLineLongerThanItem");
            trackerW = selectedButtonWidth+(selectedButtonWidth ? _itemPadding : 0);
            trackerH = _trackerHeight;
            trackerX = selectedButton.frame.origin.x;
            trackerY = self.itemScrollView.bounds.size.height - trackerH;
            //     NSLog(@"X:%f,Y:%f,W:%f,H:%f  ",trackerX,trackerY,trackerW,trackerH);
            self.tracker.frame = CGRectMake(trackerX, trackerY, trackerW, trackerH);
        }
            break;
        case SPPageMenuTrackerStyleLineAttachment:
        {
            NSLog(@"C:SPPageMenuTrackerStyleLineAttachment");
        ...
        {
            NSLog(@"D:SPPageMenuTrackerStyleRect");
            trackerW = selectedButtonWidth+(selectedButtonWidth ? _itemPadding : 0);
        ...
        {
            NSLog(@"E:SPPageMenuTrackerStyleRoundedRect");
    ...
    //    NSLog(@"----------------------selectedButton.center.x:%f  ",selectedButton.center.x);
}
#pragma mark - public
...
- (void)buttonInPageMenuClicked:(SPItem *)sender {
   ...
   // NSLog(@"buttonClicked _selectedItemIndex:%f",toIndex);
...
- (void)beginMoveTrackerFollowScrollView:(UIScrollView *)scrollView {
    ...
    // NSLog(@"SV.contentOffest.x:   %f, SV.contentSize.width:    %fs SV.bounds.size.width:     %f",scrollView.contentOffset.x, scrollView.contentSize.width,scrollView.bounds.size.width);
    if (scrollView.contentOffset.x < 0 || scrollView.contentOffset.x > scrollView.contentSize.width-scrollView.bounds.size.width)
    {
        return;
    }
    //NSLog(@"beginMove _selectedItemIndex:%ld",self.selectedItemIndex);
    static int i = 0;
    if (i == 0) {
    ...
    // NSLog(@"_beginOffesetX:%f",_beginOffsetX);
    // 当前偏移量
    ...
    //NSLog(@"currentOffsetX:%f,offsetProgress:%f,progress:%f",currentOffSetX ,offsetProgress,progress);
    NSInteger fromIndex;
    NSInteger toIndex;
    ...
    if (currentOffSetX - _beginOffsetX > 0) { // 向左拖拽了
        // 求商,获取上一个item的下标
        fromIndex = currentOffSetX / scrollView.bounds.size.width;//目前的位置，从0页面开始算
        // 当前item的下标等于上一个item的下标加1
        toIndex = fromIndex + 1;   //页数变大
        if (toIndex >= self.buttons.count) {
            toIndex = fromIndex;  //超出4页
            //       NSLog(@"begin _selectedItemIndex:  %ld,fromIdex:  %ld,toIndex:  %ld",self.selectedItemIndex,fromIndex,toIndex);
        }
        //  NSLog(@"fromIndex:%ld,toIndex:%ld",fromIndex,toIndex);
    } else if (currentOffSetX - _beginOffsetX < 0) {  // 向右拖拽了
        toIndex = currentOffSetX / scrollView.bounds.size.width; //与上一行反过来计算
        fromIndex = toIndex + 1;
        progress = 1.0 - progress;   //算滑动的比率
        //   NSLog(@"fromIndex:%ld,toIndex:%ld,progress%f",fromIndex,toIndex,progress);
        //     NSLog(@"begin _selectedItemIndex:  %ld,fromIdex:  %ld,toIndex:  %ld",self.selectedItemIndex,fromIndex,toIndex);
    } else {
        progress = 1.0;  //没有什么变化的时候
    ...
    //----------------滑到页宽整数了，contentOffset滑到了整数，
    //通过[self moveTrackerWithProgress]来回currentOffetX变了，如果与fromIndex*bounds.size.width倍数不一致，直接跳到下一个if
    if (currentOffSetX == scrollView.bounds.size.width * fromIndex) {//
        //      NSLog(@"currentOffSetX :%f",currentOffSetX);
        //   NSLog(@"currentOffSetA :%f",currentOffSetX);
        progress = 1.0;
        toIndex = fromIndex;
        //   NSLog(@"A fromIndex:%ld,toIndex:%ld,progress%f",fromIndex,toIndex,progress);
        //     NSLog(@"begin _selectedItemIndex:  %ld,fromIdex:  %ld,toIndex:  %ld",self.selectedItemIndex,fromIndex,toIndex);
    }
    ...
    if (currentOffSetX == scrollView.bounds.size.width*toIndex) { // 这里toIndex==fromIndex
        i = 0;
        //     NSLog(@"currentOffSetB :%f",currentOffSetX);
        ...
        //   NSLog(@"B begin _selectedItemIndex:  %ld,fromIdex:  %ld,toIndex:  %ld",self.selectedItemIndex,fromIndex,toIndex);
        return;
    }
    // 没有关闭跟踪模式
    if (!self.closeTrackerFollowingMode) {
        //  NSLog(@"C:  %ld,fromIdex:  %ld,toIndex:  %ld",self.selectedItemIndex,fromIndex,toIndex);
...
- (void)insertItemWithTitle:(NSString *)title atIndex:(NSUInteger)itemIndex animated:(BOOL)animated {
    ...
    if (itemIndex <= self.selectedItemIndex) {
        ...
        NSLog(@"addButton:itemIndex _selectedItemIndex:%ld",self.selectedItemIndex);
    }
}
...
- (void)moveItemScrollViewWithSelectedButton:(SPItem *)selectedButton {
    // NSLog(@"B:moveItemScrollViewWithSelectedButton:");
...
- (void)moveTrackerFollowScrollView:(UIScrollView *)scrollView {
    // NSLog(@"D:moveTrackerFollowScrollView");
...
- (void)moveTrackerWithProgress:(CGFloat)progress fromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex currentOffsetX:(CGFloat)currentOffsetX beginOffsetX:(CGFloat)beginOffsetX {
    // NSLog(@"A:moveTrackerWithProgress:");
    ...
    if (self.trackerStyle == SPPageMenuTrackerStyleLine) {   //---TrackerStyle 01
        NSLog(@"01");
    ...
    } else if (self.trackerStyle == SPPageMenuTrackerStyleLineAttachment){//---TrackerStyle 02
        NSLog(@"02");
    ...
    } else if (self.trackerStyle == SPPageMenuTrackerStyleTextZoom) {//---TrackerStyle 03
        // 缩放文字
        NSLog(@"03");
        [self zoomForTitleWithProgress:progress fromButton:fromButton toButton:toButton];
    } else if (self.trackerStyle == SPPageMenuTrackerStyleRoundedRect) {//---TrackerStyle 04
        NSLog(@"04");
        newCenter.x = fromButton.center.x + xDistance * progress;
        newFrame.size.width = fromButton.frame.size.width + wDistance * progress + (_itemTitleFont.lineHeight+10)*0.5;
        self.tracker.frame = newFrame;
        self.tracker.center = newCenter;
    } else {                                                                      //---TrackerStyle 05
        //  NSLog(@"05");
        newCenter.x = fromButton.center.x + xDistance * progress;
        newFrame.size.width = fromButton.frame.size.width + wDistance * progress + _itemPadding;
        self.tracker.frame = newFrame;   //变大_itemPadding 30的距离
        self.tracker.center = newCenter; //每个button center为51.75 155.25 258.75 362.25
        // NSLog(@"newCenter.x:  %f,fromButton.center.x :  %f,,xDistance * progress : %f",newCenter.x,fromButton.center.x,xDistance * progress );
        //   NSLog(@"newFrame.size.width :  %f,_itemPadding :  %f, wDistance*progress: %f,",newFrame.size.width ,_itemPadding ,wDistance*progress );
        //     NSLog(@"fromButton.center.x %@,tracker.center: %@",NSStringFromCGRect(self.tracker.frame),NSStringFromCGPoint( self.tracker.center));
    }
    // 文字颜色渐变
...
- (void)moveTrackerWithSelectedButton:(UIButton *)selectedButton {
    //  NSLog(@"moveTrackerWithSelectedButton");
    //tracker移动的速度
...
- (void)setItems:(NSArray *)items selectedItemIndex:(NSUInteger)selectedItemIndex {
    ...
    // NSLog(@"Spmene setItem _selectedItemIndex:%ld",selectedItemIndex);
    self.insert = NO;
...
#pragma mark - KVO & observeScrollTouch&ButtonTouch
//收听子控件scrollview的touch point变化的时候
/*
-(void)observeButtonTouch:(NSNotification *)notification{
    //CGPoint acciptPoint=self.functionButton.buttonMovePoint;
    
    //测试观察接收userinfo，并导出通知里的CGFloatX&CGFloat信息
  //  NSLog(@"%@",notification.userInfo);
    NSLog(@"收到按键通知");
    //  NSNumber *obsNumX=[notification.userInfo objectForKey:@"offsetX"];
    NSNumber *obsNumY=[notification.userInfo objectForKey:@"offsetY"];
    // CGFloat obsFloatX=[obsNumX floatValue];
    CGFloat yDistance=[obsNumY floatValue];
   // NSLog(@"obsFloatY is right?%f",obsFloatY);
    
 //   CGPoint acciptPoint=self.selectedButton.buttonMovePoint;
    //    CGRect frame = self.scrollView.frame;
   // NSLog(@"button Opint addr= %p",self.selectedButton );
    CGRect FirstViewframe=self.frame;
    
    FirstViewframe.origin.y+=yDistance;
    NSLog(@"yDistance = %f",yDistance );
    //  NSLog(@"buttonFirstframe.y =-----%f",FirstViewframe.origin.y );
...
-(void)observeScrollTouch:(NSNotification *)notification{
    ...
    NSLog(@"收到Scroll通知");
    CGPoint yDistancePoint=self.itemScrollView.FscrollMovepoint;
    //    CGRect frame = self.scrollView.frame;
    CGRect FirstViewframe=self.frame;
    ...
    FirstViewframe.origin.y+=yDistancePoint.y;
    //   frameOfview.origin.y=self.scrollviewFunction.FscrollMovepoint.y+400;
    NSLog(@"yDistance.y = %f",yDistancePoint.y );
